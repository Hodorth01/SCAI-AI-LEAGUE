...
class BallDetector:
    def __init__(self, path_model=None, device='cuda'):
        ...
        self.device = device
        if path_model:
            self.model.load_state_dict(torch.load(path_model, map_location=device,weights_only=True)) # weights_only=False change to True for security reasons
    ...
    def infer_model(self, frames):
        ...
        for num in tqdm(range(2, len(frames))):
            ...
            imgs = np.rollaxis(imgs, 2, 0) # changes order of axes from (360,640,9) to (9,360,640)
